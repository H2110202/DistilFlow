"""
一键启动脚本 — 支持三种模式：
1. web    — 浏览器模式（Chainlit）
2. ide    — 本地 IDE 模式（pywebview 桌面窗口）
3. api    — 纯 API 模式（供 VS Code Continue.dev 接入）
"""
import subprocess
import sys
import os
...
def install_deps():
    print("\n📦 安装依赖...")
    run(f'"{sys.executable}" -m pip install -e ".[all]" --quiet', check=False)
    print("✅ 依赖安装完成")
...
def mode_web():
    ...
    print("  🛑 按 Ctrl+C 停止服务")
    print("=" * 60 + "\n")
    run(f'"{sys.executable}" -m chainlit run app.py --host 0.0.0.0 --port 8000', check=False)


def mode_ide():
    """本地 IDE 模式 — pywebview 桌面窗口"""
    print("\n" + "=" * 60)
    print("  🖥️ 本地 IDE 模式启动中...")
    print("  📱 桌面窗口将自动打开")
    print("  🔌 API 端口: http://127.0.0.1:8001/v1")
    print("  🛑 关闭窗口即可停止服务")
    print("=" * 60 + "\n")
...
def mode_api():
    """纯 API 模式 — 供 VS Code Continue.dev 接入"""
    print("\n" + "=" * 60)
    print("  🔌 API 模式启动中...")
    print("  📍 端点: http://127.0.0.1:8001/v1/chat/completions")
    print("  📋 Continue.dev 配置文件: continue-config.yaml")
    print("  🛑 按 Ctrl+C 停止服务")
    print("=" * 60 + "\n")
...
def main():
    ...
    if mode == "web":
        ...
        print(f"\n❌ 未知模式: {mode}")
        print("用法: python start.py [web|ide|api]")
        print("  web — 浏览器模式 (Chainlit)")
        print("  ide — 本地 IDE 模式 (pywebview 桌面窗口) [默认]")
        print("  api — 纯 API 模式 (供 VS Code Continue.dev 接入)")
        sys.exit(1)
